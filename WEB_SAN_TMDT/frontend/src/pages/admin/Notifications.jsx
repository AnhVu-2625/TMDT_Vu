...
export default function AdminNotifications() {
  ...
  const handleSingle = async () => {
    ...
    setSubmitting(true);
    try {
      await sendNotification({ ...form, userId: parseInt(form.userId) });
  ...
  return (
    <div>
      ...
      <div className="max-w-xl">
        {tab === 'single' ? (
          <motion.div key="single" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}
            ...
            <div>
              ...
              <select value={form.loaiThongBao} onChange={e => setForm(f => ({ ...f, loaiThongBao: e.target.value }))}
            ...
            <button onClick={handleSingle} disabled={submitting}
          ...
          <motion.div key="broadcast" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }}
            className="bg-gray-900 border border-gray-800 rounded-xl p-5 space-y-4">
            ...
            <div>
              <label className="text-xs text-gray-400 mb-1 block">Loại thông báo</label>
              <select value={broadcastForm.loaiThongBao} onChange={e => setBroadcastForm(f => ({ ...f, loaiThongBao: e.target.value }))}
                className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-red-500">
                {TYPES.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}
              </select>
            </div>
            <div>
              <label className="text-xs text-gray-400 mb-1 block">Tiêu đề</label>
              <input value={broadcastForm.tieuDe} onChange={e => setBroadcastForm(f => ({ ...f, tieuDe: e.target.value }))}
                placeholder="Tiêu đề thông báo..."
                className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-red-500" />
            </div>
            <div>
              <label className="text-xs text-gray-400 mb-1 block">Nội dung</label>
              <textarea value={broadcastForm.noiDung} onChange={e => setBroadcastForm(f => ({ ...f, noiDung: e.target.value }))}
                rows={4} placeholder="Nội dung thông báo..."
                className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-red-500 resize-none" />
            </div>
            <button onClick={handleBroadcast} disabled={submitting}
