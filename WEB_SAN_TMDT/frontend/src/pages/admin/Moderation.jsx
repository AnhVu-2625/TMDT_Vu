...
export default function AdminModeration() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    // Lấy báo cáo liên quan đến sản phẩm để kiểm duyệt
    getReports({ status: 'CHO_XU_LY' })
      .then(r => setItems((r.data.data || []).filter(i => i.LoaiMaThamChieu === 'SAN_PHAM' || i.LoaiBaoCao === 'SAN_PHAM_KHONG_HOP_LE')))
    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="grid gap-3">
          {items.map(item => (
            <motion.div key={item.MaBaoCao} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-300 mb-1">{item.MoTaChiTiet}</p>
                  <p className="text-xs text-gray-600">{new Date(item.NgayTao).toLocaleString('vi-VN')}</p>
                </div>
                <div className="flex gap-2 shrink-0">
                  {item.MaThamChieu && (
                    <a href={`/products/${item.MaThamChieu}`} target="_blank" rel="noreferrer"
          ...
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
            ...
            <div className="flex gap-3 justify-end">
              <button onClick={() => { setModal(null); setNote(''); }}
                className="px-4 py-2 text-sm text-gray-400 hover:text-white transition-colors">Hủy</button>
