...
export default function AdminDisputes() {
  ...
  const handleResolve = async (quyetDinh) => {
    if (!reason.trim()) {
      toast.warning('Vui lòng nhập lý do quyết định');
      return;
    }

    if (!detail?.dispute?.MaDoiTra) {
      toast.error('Chưa có thông tin tranh chấp');
      return;
    }
    ...
    try {
      ...
      toast.success(
        quyetDinh === 'DONG_Y_HOAN_TIEN'
          ? 'Đã đồng ý hoàn tiền cho người mua'
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-3 text-sm text-gray-400">
                    <span>
                      Người mua: <span className="text-gray-200 font-medium">{d.TenNguoiMua}</span>
                    </span>
                    <span>·</span>
                    <span>
                      Shop: <span className="text-gray-200 font-medium">{d.TenCuaHang}</span>
                    </span>
                    <span>·</span>
                    <span>{d.NgayTao ? new Date(d.NgayTao).toLocaleDateString('vi-VN') : ''}</span>
                  </div>
                </div>
            ...
            {loadingDetail ? (
              ...
              <>
                <div className="flex items-center justify-between p-5 border-b border-gray-800">
                  <h3 className="text-white font-semibold">Tranh chấp đơn #{detail.dispute.MaDonHang}</h3>
                ...
                <div className="p-5 space-y-4">
                  ...
                  <div className="bg-gray-800 rounded-lg p-4">
                    ...
                    <p className="text-sm text-gray-400">
                      Giá trị đơn hàng:{' '}
                      <span className="text-white font-bold text-base">{fmtMoney(detail.dispute.TienThanhToan)}</span>
                    </p>
                  </div>
                          ...
                          >
                            ...
                            <div>
                              <p className="text-sm text-white font-medium">
                                {v.LoaiVideo === 'DONG_HANG' ? 'Video đóng hàng' : 'Video mở hàng'}
                              </p>
                              <p className="text-xs text-gray-400 mt-0.5">
                                {v.NgayTai ? new Date(v.NgayTai).toLocaleDateString('vi-VN') : ''}
                              </p>
                            </div>
                          </a>
                        ))}
                        ...
                        {detail.history.map((h) => (
                          <div key={h.MaLichSu} className="flex gap-3 text-sm">
                            <span className="text-gray-500 shrink-0 font-medium">
                              {h.NgayThucHien ? new Date(h.NgayThucHien).toLocaleDateString('vi-VN') : ''}
                            </span>
                            <div>
